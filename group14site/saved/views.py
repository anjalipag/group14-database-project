...
def see_recommendation_details(request, rec_id):
   ...
   return render(request, 'saved_rec_details.html', {'rec': details,})
...
def get_rec_details(rec_id):
   details = query_db("SELECT * FROM savedrecommendations JOIN users ON user_posted_id = user_id NATURAL JOIN recommendationpost NATURAL JOIN recommendeditem NATURAL JOIN groups WHERE recommendation_id = %s", rec_id)
   return details

def get_comments(rec_id):
   comments = query_db("SELECT * FROM comment NATURAL JOIN users WHERE recommendation_id = %s",rec_id)
   return comments
